...
const BlogDetails = async ({ params }: { params: ParamsProps }) => {
  const slug = (await params).params.slug;
  const wixClient = await wixClientServer();

  // Fetch main blog post
  const blogRes = await wixClient.items
    .query("MyBlogPosts")
    .eq("slug", slug)
    .find();
  ...
  const readTime =
    blogPost.readTime || Math.ceil(blogPost.content.split(" ").length / 200);
    ...
    <div className="min-h-screen bg-[#FAF8F1]">
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="flex flex-wrap justify-center gap-2 mb-6">
            {blogPost.categories?.map((category) => (
              <span
                key={category}
                className="px-3 py-1 text-sm font-medium text-[#D4AF37] bg-black bg-opacity-60 rounded-full border border-[#D4AF37]"
              >
                {category}
              </span>
            ))}
          ...
          <div className="flex flex-col md:flex-row items-center justify-center gap-4 text-[#E5E5E5]">
            ...
            <div className="text-center md:text-left">
              <p className="font-medium text-white">
                {blogPost.author?.name || "Wig Expert"}
              </p>
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="mt-12">
            ...
            <div className="flex flex-wrap gap-2">
              {blogPost.tags.map((tag) => (
                <span
                  key={tag}
                  className="px-3 py-1 bg-[#EDE8D5] text-[#5C5C5C] rounded-full text-sm"
                >
                  #{tag}
                </span>
              ))}
        ...
        <div className="mt-12 pt-8 border-t border-[#E0D6B7]">
          <h3 className="text-sm font-semibold text-[#5C5C5C] mb-4">
            SHARE THIS ARTICLE
          </h3>
            ...
            <div className="flex flex-col md:flex-row gap-6 bg-[#F5F1E6] p-6 rounded-lg">
              ...
              <div>
                ...
                <p className="text-[#5C5C5C]">
                  {blogPost.author.bio ||
                    "Wig care specialist with years of experience helping clients find their perfect look."}
                </p>
              </div>
            </div>
          </div>
